...
extern const md5 e_HashSig = *(md5*)&".SectionHashSig";

extern const CHAR e_pszSections[][8] = {
	".rdata\0",   // Special
	".rsrc\0\0",  // Ignore
	// ".reloc\0" // Ignore (not sure if UpdateResource might mess with this)
};
extern const size_t e_nSections = sizeof(e_pszSections) / sizeof(*e_pszSections);
...
extern const CHAR e_Base64Table[] = { "!§$%&/()?{[]}+*@~#qwertyuiopQWERTYUIOP<,.->;:_|asdfghjklASDFGHJK" };
#else
