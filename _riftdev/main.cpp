...
// This Doubly-Linked list supports only supports a maximum size of 2^16 - 16byte head
...
public:
	...
	// TODO: this is broken vor whatever reason, has to be fixed
	// or well, atleast it causes the programm to break
		...
		if (entry->size) { // Make sure entry is valid
			if (entry->next && entry->prev) {
				...
				if (entry->next) { // Remove First
					...
					((ListEntry*)((ptr)entry + entry->prev))->next = NULL;
...
private:
	...
	ListEntry* FindCave(
		...
		while (entry != m_LastEntry()) {
			if ((entry->next - (word)((ptr)entry - m_BaseAddress)) - entry->size >= nSize) {
				prev = entry, next = (ListEntry*)((ptr)entry + entry->next);
				return (ListEntry*)((ptr)entry + (entry->size + sizeof(ListEntry)));
			} else
				entry = (ListEntry*)((ptr)entry + (entry->size + sizeof(ListEntry)));
		}
	...
	inline void m_FirstEntry(_In_opt_ ListEntry* le) {
		*(word*)(m_BaseAddress + 0) = le ? (word)((ptr)le - m_BaseAddress) : NULL;
	}
	...
	inline void m_LastEntry(_In_opt_ ListEntry* le) {
		*(word*)(m_BaseAddress + 2) = le ? (word)((ptr)le - m_BaseAddress) : NULL;
	}
...
int main() {
	...
	char a[] = "Hello";
	liste[0] = list.AddObject(a, sizeof(a));
	char b[] = "HELLO YOU asshole";
	liste[1] = list.AddObject(b, sizeof(b));
	char c[] = "This is a Test";
	liste[2] = list.AddObject(c, sizeof(c));
