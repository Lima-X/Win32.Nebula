...
namespace are { // Anti Reverse Engineering
	...
	namespace dli { // Anti Dll(Module) Injection (Detection)
		...
		static HMODULE(WINAPI* RLoadLibraryExW)(_In_ LPCWSTR lpLibFileName, _Reserved_ HANDLE m_hFile, _In_ dword dwFlags) = LoadLibraryExW;
		HMODULE WINAPI HLoadLibraryExW(_In_ LPCWSTR lpLibFileName, _Reserved_ HANDLE m_hFile, _In_ dword dwFlags) {
			for (uchar i = 0; i < sizeof(l_AllowedLibraries) / sizeof(PCWSTR); i++)
				if (!wcsstr(lpLibFileName, l_AllowedLibraries[i]))
					return RLoadLibraryExW(lpLibFileName, m_hFile, dwFlags);
			return NULL;
		}
